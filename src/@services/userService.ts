...
import type { EpisodeInstance } from '../models/Episode'
...
export const userService = {
  ...
  update: async (
    ...
    const [afectedRows, updatedUsers] = await User.update(attributes, {
      where: {
        id,
      },
      returning: true,
  ...
  updatePassword: async (id: number, password: string) => {
    const [afectedRows, updatedUsers] = await User.update(
      { password },
      {
        where: {
          id,
